...
class VoxelNet(nn.Module):
    ...
    #@pysnooper.snoop('/home/qichen/Downloads/second.pytorch/second/model_sec_car_37144/predict_test/step_37144/log/file.log')
    def forward(self, example):
        ...
        print (batch_anchors.shape[0])
        t = time.time()
        # features: [num_voxels, max_num_points_per_voxel, 7]
        # num_points: [num_voxels]
        # coors: [num_voxels, 4]
        voxel_features = self.voxel_feature_extractor(voxels, num_points)
        #with open("/home/qichen/Downloads/second.pytorch/second/model_sec_car_37144/predict_test/step_37144/log/voxel_features_20.pkl", 'wb') as f:
        #    pickle.dump(voxel_features, f)
        #voxel_features += voxel_features
        #print (voxel_features[1,:])
        ...
        if self._use_sparse_rpn:
            ...
            #print (coors[2000:2049,:])
            print (max(spatial_features[0,10,:,1]))
            '''
            with open("/home/qichen/Downloads/second.pytorch/second/model_sec_car_37144/predict_test/step_37144/log/spatial_features_20.pkl", 'wb') as f1:
                pickle.dump(spatial_features, f1)
            
            with open("/home/qichen/Downloads/second.pytorch/second/model_sec_car_37144/predict_test/step_37144/log/spatial_features_99.pkl", 'rb') as f:
                #75+99
                #pickle.dump(spatial_features, f)
                spatial_features1 = pickle.load(f)
            print (max(spatial_features1[0,10,:,1]))
            #spatial_features = spatial_features + spatial_features1
            #spatial_features += spatial_features
            for i in range (128):
                for j in range (314):
                    spatial_features[:,i,:,j+19] = spatial_features[:,i,:,j+19] + spatial_features1[:,i,:,j]
            print (max(spatial_features[0,10,:,1]))
            
            #20+75
            with open("/home/qichen/Downloads/second.pytorch/second/model_sec_car_37144/predict_test/step_37144/log/spatial_features_75.pkl", 'rb') as f:
                #pickle.dump(spatial_features, f)
                spatial_features1 = pickle.load(f)
            print (max(spatial_features1[0,10,:,1]))
            #spatial_features = spatial_features + spatial_features1
            ...
            for j in range (73):
                for i in range (395):
                    spatial_features[:,55:99,i+5,72-j] = torch.max(spatial_features[:,55:99,i+5,72-j],spatial_features1[:,55:99,394-i,j])
                    #spatial_features[:,50:90,i+5,72-j] = spatial_features[:,50:90,i+5,72-j] + spatial_features1[:,50:90,394-i,j]
            ...
            if self._use_bev:
                ...
                #print (preds_dict["box_preds"].shape)
